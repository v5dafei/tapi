...
class SystemController extends BaseController
{
    ...
    public function websiteMultipleSave()
    {
        ...
        if(isset($input['version']) && !empty($input['version'])){
            $nearlyPrefixVersion = PrefixVersion::where('carrier_id',$this->carrier->id)->where('prefix',$input['prefix'])->orderBy('id','desc')->first();
            if($nearlyPrefixVersion->id != $input['version']){
                return returnApiJson('对不起您的版本过旧请更新后再保存',0);
            }
        }
        ...
        $prefixVersion->carrier_id = $this->carrier->id;
        $prefixVersion->prefix     = $input['prefix'];
        $prefixVersion->save();
...
//        ]);
    ...
    public function allPrefixSetting()
    {
        ...
        $prefixVersion = PrefixVersion::where('carrier_id',$this->carrier->id)->where('prefix',$input['prefix'])->orderBy('id','desc')->first();
        ...
        $data =[
            ...
            ['title'=>'version','arr'  =>$prefixVersion->id],
