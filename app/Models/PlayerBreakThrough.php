...
class PlayerBreakThrough extends Model
{
   
    public $table = 'log_player_break_through';
    ...
    public static function activitiesBreakThroughPlayerList($carrier)
    {
        $input          = request()->all();
        ...
        $query          = self::where('carrier_id',$carrier->id)->where('prefix',$input['prefix'])->orderBy('day','desc');
