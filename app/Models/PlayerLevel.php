...
class PlayerLevel extends Model
{
    ...
    public function playerGradeAdd($carrier)
    {
        ...
        if($this->is_system == 1){
            ...
            if($this->id){
                $sameSort  = self::where('carrier_id',$carrier->id)->where('prefix',$this->prefix)->where('is_system',2)->where('sort',$input['sort'])->first();
                if($sameSort && $sameSort->id != $this->id){
                    return '对不起，非系统分组不能有相同的排序字段';
                }
            } else {
                $sameSort  = self::where('carrier_id',$carrier->id)->where('prefix',$input['prefix'])->where('is_system',2)->where('sort',$input['sort'])->first();
                if($sameSort){
                    return '对不起，非系统分组不能有相同的排序字段';
                }
            }
