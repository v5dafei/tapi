...
class Lunar
{
    ...
    private $zodiacArr = array('鸡','狗','猪','鼠','牛','虎','兔','龙','蛇','马','羊','猴');
    ...
    public function GetSMon($year,$month)
    {
    ...
    public function YearToSx($year){
        $array =$this->zodiacArr;
        $flag = intval($year)%12;
        if($flag==0){
            return end($array);//因为数组不存在-1键/下标，所以将指针移动最后
        }else{
            return $array[$flag-1];//数组下标从0开始
        }
    }
}