...
use Core\Exception\ErrParams;

/**
 * 命令行参数解析工具类
 *
 * @link   https://blog.csdn.net/u012302539/article/details/62045208
 * @author guolinchao
 */
class Commander
{
    ...
    public function option ( $opt, callable $callback ) {
        ...
        if ( !is_callable($callback) ) {
            throw new ErrParams(sprintf('Not a valid callback <%s>.', $callback));
        }
        if ( isset(self::$optsArr[$opt]) ) {
            call_user_func($callback, self::$optsArr[$opt]);
        } else {
            throw new ErrParams(sprintf('Unknown option <%s>.', $opt));
        }
    }
