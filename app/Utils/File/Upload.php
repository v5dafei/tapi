 ...
 * PHP文件上传类
 *
 * @link  : https://github.com/aileshe/Upload/blob/master/src/Upload/Upload.php
...
class Upload
{
    ...
    public $form_name = 'file';    # 文件表单name值(string)
     ...
     * 文件接收入口 - 单文件、多文件上传
     *
     * @param  String $form_name 提交表单name
     */
    public function __construct ( $config = [] ) {
        ...
        if ( empty($this->rootPath) ) $this->rootPath = $this->rootPath;
    }
    ...
    public function save ( $storage = NULL, $allow = NULL, $host = NULL ) {
        ...
        if ( !empty($_FILES) && isset($_FILES[$this->form_name]) ) {
            # 判断存储目录是否存在,无则自动创建
            if ( !is_dir($storage) ) {
            ...
            $files = $_FILES[$this->form_name]; # 简化数组
            ...
            if ( count($filelist) === 1 ) {
                ...
                $new_arr['url']      = str_replace($this->rootPath, $this->host, $new_arr['savepath']); # 文件访问URL地址
                $new_arr['uri']      = str_replace($this->rootPath, '', $new_arr['savepath']); # 文件访问URI相对地址
                ...
                foreach ( $filelist as $v ) {
                    ...
                    $new_arr[] = array(
                        ...
                        'url'      => str_replace($this->rootPath, $this->host, $savepath), # 文件访问URL地址
                        'uri'      => str_replace($this->rootPath, '', $savepath), # 文件访问URI地址
     ...
     * @return String
     */
    private function uuid ( $size = 10 ) {
        $chars    = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
        $rand_str = null;
        for ( $i = 0; $i < $size; $i++ ) {
            $rand_str .= $chars[mt_rand(0, 61)];
        }
        return $rand_str .'_'. time();
    }
