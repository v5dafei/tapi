...
class PlayerBankCardObserver
{
    public function created(PlayerBankCard $playerBankCard)
    {
        $player                           = Player::where('player_id',$playerBankCard->player_id)->first();
        //注册赚送彩金
        ...
        if($player->is_tester ==0 && $player->type==2 && $isRegistergift && $isBindBankCard==1 && !$existRegisterGift && !$existRecharge){
                ...
                $existRecord            =  PlayerTransfer::where('player_id',$playerBankCard->player_id)->count();

                //判断是否有在领了体验券不充值的银行卡列表中
                $bankStatFlag = BankStat::where('banknumber',$playerBankCard->card_account)->first();
