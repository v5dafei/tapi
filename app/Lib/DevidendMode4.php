...
//代理自玩业绩算上级，代理下一级算直属。未开分红的会员推的会员也算直属。开了分红的下级推的人算团队  //分红=充-提-库存-场馆 摇钱树
class DevidendMode4{
    ...
    public static function calculateDividend($user,$startDate=null,$endDate=null)
    {
        ...
        $teamDividend                              = 0;
        $teamVenuesFee                             = 0;
        $calculateDirectlyunderRechargeAmount      = 0;
        ...
        if(!is_null($directPlayerCommissionAmount)){
            $directlyunderRechargeAmount          += $directPlayerCommissionAmount;
            $calculateDirectlyunderRechargeAmount += $directPlayerCommissionAmount;
        }

        $directlyunderDividend       = bcdiv(($calculateDirectlyunderRechargeAmount*$operatingExpenses- $directlyunderWithdrawAmount- $data['directlyunder_stock_change']-$venuesFee)*$selfPlayerSetting->earnings,100,2) ;
        ...
        foreach ($directlyUnderPlayerSettings as $k3 => $v3) {
            ...
            foreach ($summaryTeamRecharge as $key => $value) {
                if($playerSons[$value->player_id]== $v3->player_id){
                    ...
                    $calculateRechargeAmount   += $value->amount;
                }
            }
            ...
            foreach ($summaryTeamNoWalletRecharge as $key => $value) {
                if($playerSons[$value->player_id]== $v3->player_id){
                    $teamRechargeAmount         = $teamRechargeAmount - bcdiv($value->amount*$noWalletPassageRate,100,0);
                    $calculateRechargeAmount    = $calculateRechargeAmount - bcdiv($value->amount*$noWalletPassageRate,100,0);;
                }
            }
            ...
            foreach ($summaryTeamWalletRecharge as $key => $value) {
                if($playerSons[$value->player_id]== $v3->player_id){
                    ...
                    $calculateRechargeAmount     -= bcdiv($value->amount*$walletPassageRate,100,0);;
                }
            }
            ...
            $teamDividend              += bcdiv(($calculateRechargeAmount*$operatingExpenses - $tempWithdrawAmount- $tempStock + $tempStartStock -$teapNotReceived-$teamVenuesFee)*($selfPlayerSetting->earnings - $v3->earnings),100,2); 
        }
        ...
        //$data['directlyunderRecharge']      = $calculateDirectlyunderRechargeAmount;
        $data['directlyunderRecharge']      = $calculateDirectlyunderRechargeAmount;
