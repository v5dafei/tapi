...
//代理自玩业绩算上级，代理下一级算直属。未开分红的会员推的会员也算直属。开了分红的下级推的人算团队  //分红=(充-提-库存)*分红比例-运营费  凯博娱乐
class DevidendMode3{
    ...
    public static function promoteAndMakeMoney($input,$user)
    {
        ...
        $underDirectPlayerIds          = array_unique($subordinateDirectPlayerIds);
        ...
        //会员等级
        $playerGrades                  = CarrierPlayerGrade::where('carrier_id',$user->carrier_id)->where('prefix',$user->prefix)->orderBy('sort','asc')->get();

        foreach ($playerGrades as $key => $value) {
            $row                  = [];
            $row['key']           = $value->id;
            $row['value']         = $value->level_name;
            $row['updategift']    = $value->updategift;
            $data['levelgroup'][] = $row;
        }

        $currplayerGrades         = CarrierPlayerGrade::where('id',$user->player_level_id)->first();

        $data['levelimg']         = $currplayerGrades->sort;

        //自已的保底金额
        $selfPlayerSetting   = PlayerCache::getPlayerSetting($user->player_id);

        //今日直属业绩
        $playerBetFlowMiddle = PlayerBetFlowMiddle::select(\DB::raw('sum(agent_process_available_bet_amount) as agent_process_available_bet_amount'),'player_id')->where('created_at','>=',date('Y-m-d').' 00:00:00')->where('whether_recharge',1)->whereIn('player_id',$underDirectPlayerIds)->get();
        ...
        $playerBetFlowMiddles         = PlayerBetFlowMiddle::select(\DB::raw('sum(agent_process_available_bet_amount) as available_bet_amount'),'rid')->where('carrier_id',$user->carrier_id)->where('day',date('Ymd'))->where('rid','like',$user->rid.'|%')->where('whether_recharge',1)->groupBy('rid')->get();
    ...
    public static function myDirectlyunder($input,$user)
    {
        ...
        foreach ($items as $k => &$v) {
            ...
            $v->login_at  = !is_null($v->login_at)? date('Y-m-d',strtotime($v->login_at)):'';
        }
    ...
    public static function myTeam($input,$user)
    {
        ...
        $query                = Player::select('player_id','extend_id','user_name','day','descendantscount','is_online','login_at')->where('parent_id',$user->player_id);
    ...
    public static function calculateDividend($user,$startDate=null,$endDate=null)
    {
        ...
        $operatingExpenses                                 = bcdiv($operatingExpenses,100,2);
        ...
        //查询所有的直属  下一级及未开保底推的下级人员
        $allRids                           = PlayerSetting::where('rid','like',$user->rid.'|%')->pluck('rid')->toArray();
        $subordinateDirectPlayerIds        = PlayerSetting::where('parent_id',$user->player_id)->pluck('player_id')->toArray();
        $grandsonPlarIds                   = [];
        $directlyunderPlayerIds            = $subordinateDirectPlayerIds;


        $unEaringDirectPlayerIds           = PlayerSetting::where('parent_id',$user->player_id)->where('earnings',0)->pluck('player_id')->toArray();

        foreach ($allRids as $key => $value) {
            foreach ($unEaringDirectPlayerIds as $key1 => $value1) {
                if(strpos($value,strval($value1)) !== false ){
                    $arr = explode('|',$value);
                    $grandsonPlarIds[] = intval(end($arr));
                }
        ...
        $directlyunderRecharge  =  PlayerTransfer::select(\DB::raw('sum(amount) as amount'))->where('day','>=',$startDate)->where('day','<=',$endDate)->whereIn('type',['recharge','dividend_from_parent','agent_reimbursement'])->whereIn('player_id',$directlyunderPlayerIds)->first();
        ...
        //设置了非钱包充值点位
        if($noWalletPassageRate > 0){
            //非钱包充值金额
            $directlyunderNoWalletRechargeAmount                         =  PlayerDepositPayLog::where('is_wallet_recharge',0)->where('status',1)->where('review_time','>=',strtotime($startDate))->where('review_time','<',strtotime($endDate)+86400)->whereIn('player_id',$directlyunderPlayerIds)->sum('amount'); 
            if($directlyunderNoWalletRechargeAmount){
                $calculateDirectlyunderRechargeAmount     =  $directlyunderRechargeAmount - bcdiv($directlyunderNoWalletRechargeAmount*$noWalletPassageRate,100,0);
            } else{
                $calculateDirectlyunderRechargeAmount     =  $directlyunderRechargeAmount;
            }
        } else{
            $calculateDirectlyunderRechargeAmount     =  $directlyunderRechargeAmount;
        }
        ...
        //设置了钱包充值点位
        if($walletPassageRate>0){
            //钱包充值金额
            $directlyunderWalletRechargeAmount                         =  PlayerDepositPayLog::where('is_wallet_recharge',1)->where('status',1)->where('review_time','>=',strtotime($startDate))->where('review_time','<',strtotime($endDate)+86400)->whereIn('player_id',$directlyunderPlayerIds)->sum('amount'); 

            if($directlyunderWalletRechargeAmount){
                $calculateDirectlyunderRechargeAmount     -=  bcdiv($directlyunderWalletRechargeAmount*$walletPassageRate,100,0);
            } 
        }

        $directPlayerCommissionAmount                   = PlayerCommission::where('parent_id',$user->player_id)->where('day','>=',$startDate)->where('day','<=',$endDate)->sum('amount');
        ...
        //直属提现
        $directlyunderWithdraw =  PlayerTransfer::select(\DB::raw('sum(amount) as amount'))->where('type','withdraw_finish')->where('day','>=',$startDate)->where('day','<=',$endDate)->whereIn('player_id',$directlyunderPlayerIds)->first();
        ...
        if(is_null($playerCommissionGift->amount)){
            $playerCommissionGift->amount = 0;
        }

        //自已的流水佣金
        $selfCommissionGift      = PlayerCommission::select(\DB::raw('sum(amount) as amount'))->where('day','>=',$startDate)->where('day','<=',$endDate)->where('player_id',$user->player_id)->first();
        if(is_null($selfCommissionGift->amount)){
            $selfCommissionGift->amount = 0;
        }

        //佣金
        if($playerCommissionGift && !is_null($playerCommissionGift->amount)){
            $directlyunderCommission   = $playerCommissionGift->amount;
        }

        if(date('Ymd') <= $endDate){
            $summaryUnderEndStock          =  ReportPlayerStatDay::select(\DB::raw('sum(self_stock) as self_stock'))->where('day',date('Ymd'))->whereIn('player_id',$directlyunderPlayerIds)->first();
        } else{
            $summaryUnderEndStock          =  ReportPlayerStatDay::select(\DB::raw('sum(self_stock) as self_stock'))->where('day',$endDate)->whereIn('player_id',$directlyunderPlayerIds)->first();
        } 

        $summaryUnderStartStock            =  ReportPlayerStatDay::select(\DB::raw('sum(self_stock) as self_stock'))->where('day',date('Ymd',strtotime($startDate)-86400))->whereIn('player_id',$directlyunderPlayerIds)->first();

        if($summaryUnderEndStock && !is_null($summaryUnderEndStock->self_stock)){
            if($summaryUnderStartStock && !is_null($summaryUnderStartStock->self_stock)){
                $data['directlyunder_stock_change'] = $summaryUnderEndStock->self_stock -$summaryUnderStartStock->self_stock;
            } else{
                $data['directlyunder_stock_change'] = $summaryUnderEndStock->self_stock;
            }
        } else{
            $data['directlyunder_stock_change'] = 0;
        }
        ...
        //设置了彩金费
        if($bonusRate>0){
            $companyWinAmount = PlayerBetFlowMiddle::where('day','>=',$startDate)->where('day','<=',$endDate)->whereIn('player_id',$directlyunderPlayerIds)->sum('company_win_amount');
            if($companyWinAmount>0){
                $venuesFee = bcdiv($companyWinAmount*$bonusRate*100,1,0);
            }
        }

        //营收
        $revenue                     = $calculateDirectlyunderRechargeAmount- $directlyunderWithdrawAmount - $data['directlyunder_stock_change'] - $directlyunderCommission;
        $data['revenue']             = $revenue;
        $directlyunderDividend       = bcdiv($revenue*$selfPlayerSetting->earnings,100,2) - ($revenue*$operatingExpenses) - $selfCommissionGift->amount-$venuesFee;
        
        $allPerson                   = Player::where('rid','like',$user->rid.'|%')->pluck('player_id')->toArray();
        $teamPlayerIds               = array_diff($allPerson, $directlyunderPlayerIds);
        ...
        //直属充值
        $summaryTeamRecharge             =  PlayerTransfer::select('amount','player_id')->where('day','>=',$startDate)->where('day','<=',$endDate)->whereIn('type',['recharge','dividend_from_parent','agent_reimbursement'])->whereIn('player_id',$teamPlayerIds)->get(); 

        //三方钱包充值金额
        if($noWalletPassageRate > 0){
            $summaryTeamNoWalletRecharge = PlayerDepositPayLog::select('amount','player_id')->where('is_wallet_recharge',0)->where('status',1)->where('review_time','>=',strtotime($startDate))->where('review_time','<',strtotime($endDate)+86400)->whereIn('player_id',$teamPlayerIds)->get(); 
        }

        //钱包充值金额
        if($walletPassageRate>0){
            $summaryTeamWalletRecharge =  PlayerDepositPayLog::select('amount','player_id')->where('is_wallet_recharge',1)->where('status',1)->where('review_time','>=',strtotime($startDate))->where('review_time','<',strtotime($endDate)+86400)->whereIn('player_id',$teamPlayerIds)->get(); 
        }
        
        //直属提现
        $summaryTeamWithdraw              =  PlayerTransfer::select('amount','player_id')->where('type','withdraw_finish')->where('day','>=',$startDate)->where('day','<=',$endDate)->whereIn('player_id',$teamPlayerIds)->get();

        $summaryCommissionGift            = PlayerCommission::select('amount','player_id')->where('day','>=',$startDate)->where('day','<=',$endDate)->where('status',0)->whereIn('player_id',$teamPlayerIds)->get();
        ...
        //设置了彩金费
        if($bonusRate>0){
            $teamcompanyWinAmount = PlayerBetFlowMiddle::select(\DB::raw('sum(company_win_amount) as amount'),'player_id')->where('day','>=',$startDate)->where('day','<=',$endDate)->whereIn('player_id',$teamPlayerIds)->groupBy('player_id')->get();
        }
        ...
        foreach ($directlyUnderPlayerSettings as $k3 => $v3) {
            ...
            $tempStock                     = 0;
            $tempStartStock                = 0;
            $tempNotReceivedCommissionGift = 0;
            $venuesFee                     = 0;
            ...
            //扣减直属非钱包充值费用
            if(isset($summaryTeamNoWalletRecharge)){
                foreach ($summaryTeamNoWalletRecharge as $key => $value) {
                    if($playerSons[$value->player_id]== $v3->player_id){
                        $teamRechargeAmount         -= bcdiv($value->amount*$noWalletPassageRate,100,0);
                        $tempRechargeAmount         -= bcdiv($value->amount*$noWalletPassageRate,100,0);
                    }
                }
            }

            //扣减直属钱包充值费用
            if(isset($summaryTeamWalletRecharge)){
                foreach ($summaryTeamWalletRecharge as $key => $value) {
                    if($playerSons[$value->player_id]== $v3->player_id){
                        $teamRechargeAmount          -= bcdiv($value->amount*$walletPassageRate,100,0);
                        $tempRechargeAmount          -= bcdiv($value->amount*$walletPassageRate,100,0);
                    }
                }
            }
            ...
            //直属未领佣金
            foreach ($summaryCommissionGift as $key => $value) {
                if($playerSons[$value->player_id]== $v3->player_id){
                    $tempNotReceivedCommissionGift += $value->amount;
                }
            }
            ...
            //扣除彩金费
            if(isset($teamcompanyWinAmount)){
                foreach ($teamcompanyWinAmount as $key => $value) {
                    if($playerSons[$value->player_id]== $v3->player_id){
                        //团队库存
                        if($value->amount > 0){
                            $venuesFee = bcdiv($value->amount*$bonusRate*100,1,0);
                        }
                    }
                }
            }
            ...
            $data['team_stock_change']  = $data['team_stock_change'] + $tempStock - $tempStartStock;
            //团队营收
            $teamrevenue                = $tempRechargeAmount - $tempWithdrawAmount - $tempStock + $tempStartStock - $tempNotReceivedCommissionGift-$venuesFee;
            $data['team_revenue']       += $teamrevenue;
            $teamDividend               = $teamDividend + bcdiv($teamrevenue*($selfPlayerSetting->earnings - $v3->earnings),100,2); 
        }
        ...
        $data['directlyunderRecharge']      = $directlyunderRechargeAmount + $directlyunderCommission;
    ...
    public static function singleStockCalculateMemberByday($player, $number=0) 
    {
        ...
        if(!$number){
            ...
            if($preReportPlayerStatDay){
                ...
                if(isset($reportPlayerStatDay->self_stock)){
                    return ['self_stock'=>$reportPlayerStatDay->self_stock,'change_self_stock'=>$reportPlayerStatDay->self_stock];
                } else{
                    return ['self_stock'=>0,'change_self_stock'=>0];
                }
            }
        } 
    }
    ...
    public static function stockRealCalculateDividend($maxLevel,$minLevel,$allPlayers,$carrierPreFixDomain)
    {
        ...
        do{
            ...
            foreach ($cyclePlayers as $k => $v) {
                ...
                $realVenueFeeAmount            = PlayerRealCommission::where('player_id',$v->player_id)->where('day',date('Ymd'))->sum('amount');

                if(!is_null($realVenueFeeAmount)){
                    $reportRealPlayerEarnings->venue_fee                            = $result['venue_fee']+$realVenueFeeAmount;
                    $reportRealPlayerEarnings->amount                               = $result['totalCommission'] + $result['lastaccumulation'] - $realVenueFeeAmount;
                } else{
                ...
                $reportRealPlayerEarnings->directlyunderVenuesFee               = $result['directlyunderVenuesFee'];
                $reportRealPlayerEarnings->teamVenuesFee                        = $result['teamVenuesFee'];
