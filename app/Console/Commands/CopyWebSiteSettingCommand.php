...
class CopyWebSiteSettingCommand extends Command {
    ...
    public function handle()
    {
        ...
        //会员等级配对
        $data                           = [];
        $carrierPayChannelIds           = CarrierPayChannel::where('prefix',self::COPYPREFIX)->pluck('id')->toArray();
        $carrierPlayerLevelBankCardMaps = CarrierPlayerLevelBankCardMap::whereIn('carrier_channle_id',$carrierPayChannelIds)->get();

        foreach ($carrierPlayerLevelBankCardMaps as $key => $value) {
            $row                         = [];
            $row['carrier_id']           = $value->carrier_id;
            $row['carrier_channle_id']   = $mapPayChannelId[$value->carrier_channle_id];
            $row['player_level_id']      = $value->player_level_id;
            $row['created_at']           = $value->created_at;
            $row['updated_at']           = $value->updated_at;
            $data[]                      = $row;
        }
